...
Я пока не стал бороться с обновлениями cnchi, а просто решил использовать заведомо шустрые зеркала.
...
При моих 24 GiB RAM получается, что с диска на swap+ESP+boot уйдёт 24 + 0.5 + 4 = 28.5 GiB. Эм... это как бы даже больше, чем обычно заполненый root-раздел. Ну да ладно.
...
Дальше я считаю, что тот самый диск, на который ставим систему - sda и что на нем нет и не будет других ОС. Для однократной разбивки можно было бы пользоваться `gparted` или `cfdisk`/`gfdisk`, но я хочу сделать воспроизводимую установку. Так что только `parted`. 
